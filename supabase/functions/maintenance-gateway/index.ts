 ...
 * البوابة الرسمية أصبحت /functions/v1/gateway. هذه الدالة تمرّر الطلبات كما هي
 * (REST بقنوات الصيانة، أو action للبوتات، أو JSON-RPC للـ MCP).
 */
...
Deno.serve(async (req) => {
  ...
  if (req.method === 'GET') {
    if (url.pathname.endsWith('/health')) return json({ ok: true, shim: 'maintenance-gateway', target: GATEWAY });
    return json({
      name: 'maintenance-gateway (deprecated shim)',
  ...
  if (req.method !== 'POST') {
    return json({ error: 'Method not allowed', message_ar: 'الطريقة غير مسموحة' }, 405);
  }
  ...
  try {
    ...
    return json({ error: 'Invalid JSON body', message_ar: 'محتوى الطلب غير صالح' }, 400);
  }
