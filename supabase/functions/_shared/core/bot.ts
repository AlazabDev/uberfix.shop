...
async function handleCreateRequest(supabase: any, supabaseUrl: string, payload: any, metadata?: any) {
  ...
  // الاستدعاء داخلي (in-process): لا نمرر Service Role كـ Bearer لأن محرك الصيانة
  // سيحاول تفسيره كتوكن OAuth للبوابة. قناة bot_gateway قناة داخلية موثوقة.
  try {
    ...
    if (gatewayResp.ok && gatewayResult.request_id) {
      // ملاحظة: محرك الصيانة يرسل إشعار واتساب الموحد بنفسه،
      // فلا نرسل رسالة ثانية من هنا لتجنب التكرار.
      return {
        success: true,
        request_id: gatewayResult.request_id,
        tracking_number: gatewayResult.request_number,
        message: `تم إنشاء الطلب بنجاح. رقم التتبع: ${gatewayResult.request_number || gatewayResult.request_id}`,
      };
    }
