...
- Callback (الرد): edge function `eta-callback` (`verify_jwt = false`) receives ETA document status notifications at `https://zrrffsjbfkphridqyais.supabase.co/functions/v1/eta-callback?token=<ETA_CALLBACK_TOKEN>` — this URL (with the token) is pasted in «رابط الاتصال» when registering the ERP in the ETA taxpayer portal. Maps Valid/Invalid/Rejected/Cancelled/Submitted to `invoices.eta_status` (matched by `eta_uuid` then `eta_internal_id`) and logs each notification to `eta_submissions` with action `notification`. Requests without the correct token are rejected (401).